          ...
          <v-card-text>
            <v-row>
              <v-col cols="12">
                ...
                <v-text-field v-model="etax.datavalue" label="数据编号" placeholder="" :rules="[rules.required]" :autofocus=true :readonly="getReadonly()" />
              </v-col>
            </v-row>
            <v-row>
              <v-col cols="12">
                <v-text-field v-model="etax.datacheacker" label="数据检查单位" placeholder="" :rules="[rules.required]" :readonly="getReadonly()" />
              </v-col>
            </v-row>
            <v-row>
              <v-col cols="6">
                <v-text-field v-model="etax.datapricedecidetime" label="数据定价的时间" placeholder="" :rules="[rules.required]" :readonly="getReadonly()" />
              </v-col>
              <v-col cols="6">
                <v-text-field v-model="etax.datasize" label="数据资产的大小" placeholder="" :rules="[rules.required]" :readonly="getReadonly()" />
              </v-col>
            </v-row>
          </v-card-text>
...
export default {
  ...
  methods: {
    ...
    saveNew() {
      ...
      let flag = false;
      this.$axios
          .get("api/assetPricing/insert", {

            params: {

              datacheacker: this.etax.datacheacker,
              datapricedecidetime: (new Date(this.etax.datapricedecidetime)).Format('yyyy-MM-dd hh:mm:ss').toString(),
              datasize: this.etax.datasize,
              datavalue: this.etax.datavalue,
              isonline: flag
            },
          })
          .then(res => {
